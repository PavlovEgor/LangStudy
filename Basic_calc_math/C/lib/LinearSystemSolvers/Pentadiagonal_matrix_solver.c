...
void pentadiagonal_solve(struct pentadiagonal_matrix* Pent, double *x, double *B){
    ...
    R[0] = B[0];
    P[0][0] = -(Pent -> first_up_diag[0]);
    P[0][k-1] = -(Pent -> second_up_diag[0]);


    for (i = 1; i < k; i++)
    {
        di = (Pent -> first_down_diag[i]);

        frac = 1 / (1 + P[i-1][0] * di);

        P[i][k-1] = - (Pent -> second_up_diag[i]) * frac;
        for (j = 0; j < k-1; j++)
        {
            P[i][j] = - di * P[i-1][j+1] * frac;
        }
        P[i][0] += -(Pent -> first_up_diag[i]) * frac;
    ...
    di = (Pent -> first_down_diag[N-1]);
    ei = (Pent -> second_down_diag[N-1]);
...
int main(){
    ...
    struct pentadiagonal_matrix test_matrix;

    test_matrix.N = 10;
    test_matrix.k = 3;

    test_matrix.first_up_diag = fud;
    test_matrix.second_up_diag = sud;
    test_matrix.first_down_diag = fdd;
    test_matrix.second_down_diag = sdd;
    ...
    pentadiagonal_solve(&test_matrix, x, B);
